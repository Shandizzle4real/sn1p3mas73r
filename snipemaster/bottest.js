...
const sellTokenIn = data.to_PURCHASE;
const sellTokenOut = data.WBNB;
  ...
  let buyAction = async() => {
    ...
    try{
      ...
      getBalance();
        ...
        //Our execution price will be a bit different, we need some flexbility
        const amountOutMin = amounts[1].sub(amounts[1].div(`${data.Slippage}`)); //ethers.utils.parseUnits(`${data.AMOUNT_OF_WBNB}`, 'ether');
      ...
      console.log(chalk.yellow(`data.gasPrice: ${data.gasPrice}`));
        ...
        const sellTx = await router.swapExactTokensForTokens( //uncomment here if you want to buy token
        amountIn,
        amountOutMin,
